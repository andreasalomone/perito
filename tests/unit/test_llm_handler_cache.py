...
from google.genai import types as genai_types # Renamed to avoid conflict with unittest.mock.types
...
class TestGetOrCreatePromptCache(unittest.TestCase):
    ...
    def test_create_cache_success_no_existing_name(self, MockGenaiClient):
        ...
        self.assertEqual(call_args[1]['config']['display_name'], settings.CACHE_DISPLAY_NAME)
        self.assertIn(PREDEFINED_STYLE_REFERENCE_TEXT, call_args[1]['config']['contents'][0].parts[0].text)
        self.assertIn(REPORT_STRUCTURE_PROMPT, call_args[1]['config']['contents'][1].parts[0].text)
        self.assertIn(SYSTEM_INSTRUCTION, call_args[1]['config']['system_instruction'].parts[0].text)
    ...
    def test_create_cache_api_error(self, MockGenaiClient):
        ...
        settings.REPORT_PROMPT_CACHE_NAME = None
        mock_client_instance = MockGenaiClient.return_value
        mock_client_instance.caches.create.side_effect = google_exceptions.InternalServerError("Creation failed via API")
    ...
    def test_retrieve_cache_success(self, MockGenaiClient):
        ...
        mock_cache_get_result.model = f"models/{settings.LLM_MODEL_NAME}" # Matches current model
    ...
    def test_retrieve_cache_not_found_then_create(self, MockGenaiClient):
        ...
        mock_client_instance = MockGenaiClient.return_value
        mock_client_instance.caches.get.side_effect = google_exceptions.NotFound("Cache not found")
        ...
        mock_client_instance.caches.get.assert_called_once_with(name="cachedContents/nonExistentCache789")
        mock_client_instance.caches.create.assert_called_once()
    ...
    def test_retrieve_cache_different_model_then_create(self, MockGenaiClient):
        ...
        self.assertEqual(result_cache_name, "cachedContents/newlyCreatedAfterModelMismatch")
        mock_client_instance.caches.get.assert_called_once_with(name=existing_cache_id)
        mock_client_instance.caches.create.assert_called_once()
        self.assertEqual(mock_client_instance.caches.create.call_args[1]['model'], "current-app-model")
    ...
    def test_retrieve_cache_other_error_then_create(self, MockGenaiClient):
        ...
        mock_client_instance = MockGenaiClient.return_value
        mock_client_instance.caches.get.side_effect = Exception("Some generic retrieval error")
        ...
        self.assertEqual(result_cache_name, "cachedContents/newlyCreatedAfterGenericError")
        mock_client_instance.caches.get.assert_called_once_with(name="cachedContents/errorCache654")
        mock_client_instance.caches.create.assert_called_once()
    ...
    def test_model_name_stripping_for_creation(self, MockGenaiClient):
        ...
        settings.REPORT_PROMPT_CACHE_NAME = None
        settings.LLM_MODEL_NAME = "models/gemini-test-model-with-prefix" # Model name with prefix
        ...
        mock_cache_create_result.model = f"models/{expected_model_id_for_creation}" # API returns with prefix
        mock_client_instance.caches.create.return_value = mock_cache_create_result
    ...
    def test_model_name_no_prefix_for_creation(self, MockGenaiClient):
        ...
        settings.REPORT_PROMPT_CACHE_NAME = None
        settings.LLM_MODEL_NAME = "gemini-test-model-no-prefix" # Model name without prefix
