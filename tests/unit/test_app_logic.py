...
def app():
    ...
    if not hasattr(flask_app, 'logger_configured_for_tests'):
        ...
        logging.basicConfig(level=logging_level, format='%(asctime)s - %(levelname)s - %(name)s - %(request_id)s - %(message)s')
...
def test_upload_no_files_selected(client):
    ...
    with client.session_transaction() as session:
        ...
        assert any("No file part in the request" in message[1] for message in session['_flashes'])
...
def test_upload_empty_filenames_in_file_list(client):
    ...
    empty_file = FileStorage(io.BytesIO(b""), filename="", content_type='application/octet-stream')
    response = client.post('/upload', data={'files[]': [empty_file]}, content_type='multipart/form-data')
    assert response.status_code == 302
    with client.session_transaction() as session:
        assert '_flashes' in session
        # This should be caught by _validate_file_list
        assert any("No files selected for uploading." in message[1] for message in session['_flashes'])
...
def test_upload_file_type_not_allowed(client):
    ...
    response = client.post('/upload', data={'files[]': [disallowed_file]}, content_type='multipart/form-data')
    assert response.status_code == 302 # Redirects back
    with client.session_transaction() as session:
        assert '_flashes' in session
        assert any("File type not allowed for test.disallowed" in message[1] for message in session['_flashes'])
...
def test_upload_single_file_exceeds_size_limit(mock_settings, client):
    ...
    response = client.post('/upload', data={'files[]': [large_file]}, content_type='multipart/form-data')
    assert response.status_code == 302 # Redirects
    with client.session_transaction() as session:
        assert '_flashes' in session
        assert any(f"File large.txt exceeds the size limit" in message[1] for message in session['_flashes'])
...
def test_upload_total_files_exceed_size_limit(mock_settings, client):
    ...
    file1 = FileStorage(io.BytesIO(file1_content), filename="file1.txt", content_type='text/plain')
    
    file2_content = b'b' * 80 
    file2 = FileStorage(io.BytesIO(file2_content), filename="file2.txt", content_type='text/plain')

    # Total size = 80 + 80 = 160, which is > MAX_TOTAL_UPLOAD_SIZE_BYTES (150)
    response = client.post('/upload', data={'files[]': [file1, file2]}, content_type='multipart/form-data')
    
    assert response.status_code == 302 # Should redirect
    with client.session_transaction() as session:
        assert '_flashes' in session
        assert any(f"Total upload size exceeds the limit" in message[1] for message in session['_flashes'])
...
def test_upload_successful_flow(
    ...
    mock_process_file.side_effect = [
        {'type': 'text', 'filename': 'file1.txt', 'content': 'Text from file1', 'source': 'file content'},
        {'type': 'text', 'filename': 'file2.pdf', 'content': 'Text from file2', 'source': 'file content'}
    ]
...
def test_upload_text_truncation(
    ...
    mock_process_file.side_effect = [
        {'type': 'text', 'filename': 'file1.txt', 'content': '1234567890', 'source': 'file content'}, # 10 chars
        {'type': 'text', 'filename': 'file2.txt', 'content': 'abcdefghijklmnop', 'source': 'file content'}, # 16 chars
        {'type': 'text', 'filename': 'file3.txt', 'content': 'XYZ', 'source': 'file content'} # 3 chars
    ]
    ...
    response = client.post('/upload', data={'files[]': [file1, file2, file3]}, content_type='multipart/form-data')

    assert response.status_code == 200 # Assuming it still proceeds to generate a report
    with client.session_transaction() as session:
        assert '_flashes' in session
        flashed_messages = [msg[1] for msg in session['_flashes']]
        assert any("Content from file2.txt (file content) was truncated" in msg for msg in flashed_messages)
        assert any("Skipped some content from file3.txt (file content)" in msg for msg in flashed_messages)

    expected_processed_files_for_llm = [
        {'type': 'text', 'filename': 'file1.txt', 'content': '1234567890', 'source': 'file content'}, # Full
        {'type': 'text', 'filename': 'file2.txt', 'content': 'abcdefghij', 'source': 'file content'}  # Truncated (10 chars from 16)
        # file3.txt content is skipped entirely
    ]

    mock_generate_report.assert_called_once_with(
        processed_files=expected_processed_files_for_llm,
        additional_text=""
    )
...
def test_upload_eml_processing(
    ...
    eml_processed_data = {
        ...
        'processed_attachments': [
            {'type': 'text', 'filename': 'attach1.txt', 'content': 'Attachment 1 text. ', 'source': 'attachment'}, # 19 chars
            {'type': 'vision', 'filename': 'image.png', 'content': 'base64_encoded_image_data', 'source': 'attachment'}, # Vision, not text
            {'type': 'text', 'filename': 'attach2.txt', 'content': 'Attachment 2 text, long enough to be truncated. ', 'source': 'attachment'} # 48 chars
        ]
    }
    ...
    response = client.post('/upload', data={'files[]': [eml_file]}, content_type='multipart/form-data')
    ...
    with client.session_transaction() as session:
        ...
        assert any("Content from attach2.txt (attachment from EML: email.eml) was truncated" in msg for msg in flashed_messages)

    expected_llm_input = [
        {'type': 'text', 'filename': 'email.eml', 'content': 'Email body text. ', 'source': 'email body'},
        {'type': 'text', 'filename': 'attach1.txt', 'content': 'Attachment 1 text. ', 'source': 'attachment from EML: email.eml'},
        {'type': 'vision', 'filename': 'image.png', 'content': 'base64_encoded_image_data', 'source': 'attachment'}, # Vision included as is
        {'type': 'text', 'filename': 'attach2.txt', 'content': 'Attachment 2 text, long enough to be truncated. '[:100-17-19], 'source': 'attachment from EML: email.eml'} # Truncated part
    ]
    ...
    expected_llm_input[3]['content'] = 'Attachment 2 text, long enough to be truncated. '[:64]

    mock_generate_report.assert_called_once_with(
        processed_files=expected_llm_input,
        additional_text=""
    )
