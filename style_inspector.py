...
def get_cell_border_info(cell):
    ...
    for border_name in ["top", "left", "bottom", "right", "insideH", "insideV"]:
        ...
        if border_el is not None:
            ...
            if size: info += f" (size: {size})"
            if color: info += f" (color: {color})"
...
def inspect_docx_styles(file_path: str):
    ...
    for i, section in enumerate(document.sections):
        ...
        print(f"  - Page Size: {section.page_width.cm:.2f}cm (width) x {section.page_height.cm:.2f}cm (height)")
    ...
    for style in sorted(styles, key=lambda s: s.name):
        if style.type == 1:  # Paragraph styles
            ...
            if p_fmt:
                ...
                line_spacing_val = f"{p_fmt.line_spacing:.2f}" if p_fmt.line_spacing is not None else "N/A"
                print(f"    - Line Spacing: {line_spacing_val} (Rule: {p_fmt.line_spacing_rule})")
                space_before_pt = f"{p_fmt.space_before.pt:.2f}pt" if p_fmt.space_before else "0pt"
                space_after_pt = f"{p_fmt.space_after.pt:.2f}pt" if p_fmt.space_after else "0pt"
                print(f"    - Spacing: {space_before_pt} before, {space_after_pt} after")
                left_indent_cm = f"{p_fmt.left_indent.cm:.2f}cm" if p_fmt.left_indent else "0cm"
                right_indent_cm = f"{p_fmt.right_indent.cm:.2f}cm" if p_fmt.right_indent else "0cm"
                first_line_indent_cm = f"{p_fmt.first_line_indent.cm:.2f}cm" if p_fmt.first_line_indent else "0cm"
                print(f"    - Indentation: Left {left_indent_cm}, Right {right_indent_cm}, First-line {first_line_indent_cm}")
            ...
            for r_idx, row in enumerate(table.rows):
                for c_idx, cell in enumerate(row.cells):
                    ...
                    for p_idx, p in enumerate(cell.paragraphs):
                        if p.text.strip():
                            print(f"      - Paragraph {p_idx+1}: '{p.text[:60].strip()}...'")
                            for r_idx_run, run in enumerate(p.runs):
                                if run.text.strip():
                                     print(f"        - Run {r_idx_run+1}: {get_font_info(run.font)}")
